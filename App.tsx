  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (dbSettings && (dbSettings.whatsapp_main || dbSettings.address)) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <main className="flex-grow">
        ...
        {/* Categories Bar */}
        <div className="bg-white border-b border-slate-200 sticky top-20 z-30 overflow-x-auto shadow-sm">
          <div className="container mx-auto px-4 py-6 flex items-center space-x-4 no-scrollbar">
            <button
              onClick={() => setActiveCategory(null)}
              className={`flex items-center space-x-3 px-6 py-3 rounded-2xl text-xs font-black transition-all whitespace-nowrap uppercase tracking-widest ${activeCategory === null ? 'bg-brand-navy text-white shadow-xl' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
                }`}
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${activeCategory === null ? 'bg-white/20' : 'bg-white'}`}>
            ...
            {categories.map(cat => (
              <button
                ...
                className={`flex items-center space-x-3 px-6 py-3 rounded-2xl text-xs font-black transition-all whitespace-nowrap uppercase tracking-widest ${activeCategory === cat.id ? 'bg-brand-red text-white shadow-xl scale-105' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
                  }`}
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center overflow-hidden border ${activeCategory === cat.id ? 'bg-white border-white/20' : 'bg-white border-slate-200'}`}>
                  <img src={resolveImageUrl(cat.icon)} alt={cat.name} className="w-5 h-5 object-contain" />
                </div>
                <span>{cat.name}</span>
              </button>
            ))}
          ...
          {isLoading ? (
            ...
            <>
              <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
                <div>
                  <h2 className="text-4xl font-black text-brand-navy mb-2 uppercase tracking-tighter">
                ...
                <span className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.2em]">{filteredProducts.length} Equipamentos Disponíveis</span>
              </div>
